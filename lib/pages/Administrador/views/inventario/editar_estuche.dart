...
import 'package:inventario/pages/Administrador/views/inventario/ver_estuches.dart';
import 'package:inventario/services/database_services.dart';
import 'package:inventario/pages/Administrador/views/inventario/lista_estuches.dart';

class EditarEstuche extends StatefulWidget {
  ...
  @override
  State<EditarEstuche> createState() => _EditarEstucheState();
}

class _EditarEstucheState extends State<EditarEstuche> {
  final _formKey = GlobalKey<FormState>();
  final _formKeyEstuche= GlobalKey<FormState>();
  ...
  try {
    print("Actualizando estuche con ID: ${widget.estucheId}");
    print("Datos: $estucheData");

    await _databaseServices.updateEstuche(widget.estucheId, estucheData);
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al actualizar estuche: $e')),
      );
    }
  } finally {
    if (mounted) {
...
void _eliminarEstuche() {
  showDialog(
    context: context,
    builder: (context) => AlertDialog(
      backgroundColor: Colors.white,
      title: const Text('Eliminar'),
      content: const Text('¿Estás seguro de que deseas eliminar este estuche? Esta acción no se puede deshacer.'),
      actions: [
        TextButton(
        ...
        ElevatedButton(
          onPressed: () async {
            try {
              await _databaseServices.deleteEstuche(widget.estucheId); 
              if (mounted) {
                Navigator.pop(context); 
                widget.onNavigate(ListaEstuches(onNavigate: widget.onNavigate)); 
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Estuche eliminado con éxito')),
                );
              }
            } catch (e) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Error al eliminar estuche: $e')),
              );
            }
          },
        ...
        children: [
          ...
          Center(
            child: Wrap(
              ...
              children: [
                _buildCardEditar(Icons.edit, 'Editar Estuche', _mostrarFormularioEdicion),
                _buildCardVer(Icons.visibility, 'Ver Estuches', () => _verEstuches(widget.estucheId, widget.marca)),
                _buildCardAgregar(Icons.add, 'Agregar Estuche', _agregarEstuche),
              ],
            ),
          ),
          ...
          Center(
            child: IconButton(
              ...
              onPressed: _eliminarEstuche,
              tooltip: 'Eliminar Estuche',
            ),
          ),
        ],
      ),
...
Widget _buildCardEditar(IconData icon, String text, VoidCallback onTap) {
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _isSaving ? null : _editarEstuche,
  ...
  void _verEstuches(String estucheId, String tipo) {
    widget.onNavigate(VerEstuches(
    onNavigate: widget.onNavigate, 
    estucheId: estucheId,
    tipo: tipo, 
  ));
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        if (!_formKeyEstuche.currentState!.validate()) return;
                        ...
                        try {
                          ...
                          if (mounted) {
                            Navigator.pop(context);
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Estuche agregado con éxito')),
                            );
                          }
                        } catch (e) {
                          if (mounted) {
