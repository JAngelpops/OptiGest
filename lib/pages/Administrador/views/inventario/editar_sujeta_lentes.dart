...
import 'package:inventario/pages/Administrador/views/inventario/ver_sujeta_lentes.dart';
import 'package:inventario/services/database_services.dart';

class EditarSujetaLentes extends StatefulWidget {
  ...
  @override
  State<EditarSujetaLentes> createState() => _EditarSujetaLentesState();
}

class _EditarSujetaLentesState extends State<EditarSujetaLentes> {
 final _formKey = GlobalKey<FormState>();
  final _formKeySujetaLentes= GlobalKey<FormState>();
  ...
  void _editarSujetaLentes() async {
  ...
  try {
    print("Actualizando sujeta lentes con ID: ${widget.sujetaLentesId}");
    print("Datos: $sujetaLentesData");

    await _databaseServices.updateSujetaLentes(widget.sujetaLentesId, sujetaLentesData);
    ...
    if (mounted) {
      ...
      widget.onNavigate(EditarSujetaLentes(
        sujetaLentesId: widget.sujetaLentesId,
        correo: correo,
        marca: marca,
        telefono: telefono,
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al actualizar el sujeta lente: $e')),
      );
    }
  } finally {
    if (mounted) {
...
void _eliminarSujetaLentes() async {
  showDialog(
    context: context,
    builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            try {
              await _databaseServices.deleteSujetaLentes(widget.sujetaLentesId);
              if (mounted) {
                Navigator.pop(context);
                widget.onNavigate(ListaSujetaLentes(onNavigate: widget.onNavigate));
                ScaffoldMessenger.of(context).showSnackBar(
...
Widget build(BuildContext context) {
        ...
        children: [
          ...
          Center(
            child: Wrap(
              ...
              children: [
                _buildCardEditar(Icons.edit, 'Editar Sujeta Lentes', _mostrarFormularioEdicion),
                _buildCardVer(Icons.visibility, 'Ver Sujeta Lentes', () => _verSujetaLentes(widget.sujetaLentesId, widget.marca)),
                _buildCardAgregar(Icons.add, 'Agregar Sujeta Lentes', _agregarSujetaLente),
              ],
            ),
          ),
          ...
          Center(
            child: IconButton(
              ...
              onPressed: _eliminarSujetaLentes,
              tooltip: 'Eliminar Sujeta Lentes',
            ),
          ),
        ],
      ),
 ...
 Widget _buildCardEditar(IconData icon, String text, VoidCallback onTap) {
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _isSaving ? null : _editarSujetaLentes,
  ...
  void _verSujetaLentes(String sujetaLentesId, String tipo) {
    widget.onNavigate(VerSujetaLentes(
    onNavigate: widget.onNavigate, 
    sujetaLentesId: sujetaLentesId,
    tipo: tipo, 
  ));
  }

  void _agregarSujetaLente() {
  showDialog(
    context: context,
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        if (!_formKeySujetaLentes.currentState!.validate()) return;
                        ...
                        try {
                          ...
                          if (mounted) {
                            Navigator.pop(context);
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Sujeta lente agregado con éxito')),
                            );
                          }
                        } catch (e) {
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Error al agregar sujeta lente: $e')),
                            );
                          }
                        }
                      },
