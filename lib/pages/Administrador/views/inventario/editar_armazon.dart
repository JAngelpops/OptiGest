...
import 'package:inventario/pages/Administrador/views/inventario/ver_armazones.dart';
...
void _eliminarArmazon() {
  showDialog(
    context: context,
    builder: (context) => AlertDialog(
      backgroundColor: Colors.white,
      title: const Text('Eliminar Armazón'),
      content: const Text('¿Estás seguro de que deseas eliminar este armazón? Esta acción no se puede deshacer.'),
      actions: [
        TextButton(
        ...
        ElevatedButton(
          onPressed: () async {
            try {
              await _databaseServices.deleteArmazon(widget.armazonId); // Elimina el documento en Firestore
              if (mounted) {
                Navigator.pop(context); 
                widget.onNavigate(ListaArmazones(onNavigate: widget.onNavigate)); // 🔄 Redirige
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Armazón eliminado con éxito')),
                );
              }
            } catch (e) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Error al eliminar armazón: $e')),
              );
            }
          },
        ...
        children: [
          ...
          Center(
            child: Wrap(
              ...
              children: [
                _buildCardEditar(Icons.edit, 'Editar Armazón', _mostrarFormularioEdicion),
                _buildCardVer(Icons.visibility, 'Ver Lentes', () => _verLentes(widget.armazonId, widget.marca)),
                _buildCardAgregar(Icons.add, 'Agregar Lente', _agregarLente),
              ],
            ),
          ),
