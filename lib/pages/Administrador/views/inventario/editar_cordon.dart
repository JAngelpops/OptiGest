...
import 'package:inventario/pages/Administrador/views/inventario/lista_cordones.dart';
import 'package:inventario/pages/Administrador/views/inventario/ver_cordones.dart';
import 'package:inventario/services/database_services.dart';

class EditarCordon extends StatefulWidget {
  final String cordonId;
  final String marca;
  final String correo;
  final String telefono;
  final  Function(Widget) onNavigate;
  const EditarCordon({super.key,
  ...
  @override
  State<EditarCordon> createState() => _EditarCordonState();
}

class _EditarCordonState extends State<EditarCordon> {
  final _formKey = GlobalKey<FormState>();
  final _formKeyCordon= GlobalKey<FormState>();
  ...
  try {
    print("Actualizando cordon con ID: ${widget.cordonId}");
    print("Datos: $cordonData");

    await _databaseServices.updateCordon(widget.cordonId, cordonData);
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al actualizar cordon: $e')),
      );
    }
  } finally {
    if (mounted) {
...
void _eliminarCordon() {
  showDialog(
    context: context,
    builder: (context) => AlertDialog(
      backgroundColor: Colors.white,
      title: const Text('Eliminar'),
      content: const Text('¿Estás seguro de que deseas eliminar este cordón? Esta acción no se puede deshacer.'),
      actions: [
        TextButton(
        ...
        ElevatedButton(
          onPressed: () async {
            try {
              await _databaseServices.deleteCordon(widget.cordonId); // Elimina en Firestore
              if (mounted) {
                Navigator.pop(context); 
                widget.onNavigate(ListaCordones(onNavigate: widget.onNavigate)); // 🔄 Redirige
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Cordón eliminado con éxito')),
                );
              }
            } catch (e) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Error al eliminar cordón: $e')),
              );
            }
          },
...
Widget build(BuildContext context) {
  return Scaffold(
        ...
        children: [
          ...
          Center(
            child: Wrap(
              ...
              children: [
                _buildCardEditar(Icons.edit, 'Editar Cordón', _mostrarFormularioEdicion),
                _buildCardVer(Icons.visibility, 'Ver Cordones', () => _verCordones(widget.cordonId, widget.marca)),
                _buildCardAgregar(Icons.add, 'Agregar Cordón', _agregarCordon),
              ],
            ),
          ),
          ...
          Center(
            child: IconButton(
              ...
              onPressed: _eliminarCordon,
              tooltip: 'Eliminar Cordón',
            ),
          ),
        ],
      ),
    ...
    floatingActionButton: FloatingActionButton(
      elevation: 0,
      onPressed: () {
        widget.onNavigate(ListaCordones(onNavigate: widget.onNavigate));
      },
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _isSaving ? null : _editarCordon,
  ...
  void _verCordones(String cordonId, String tipo) {
    widget.onNavigate(VerCordones(
    onNavigate: widget.onNavigate, 
    cordonId: cordonId,
    tipo: tipo, 
  ));
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        if (!_formKeyCordon.currentState!.validate()) return;
                        ...
                        try {
                          ...
                          if (mounted) {
                            Navigator.pop(context);
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Cordon agregado con éxito')),
                            );
                          }
                        } catch (e) {
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('Error al agregar estuche: $e')),
                            );
                          }
                        }
                      },
