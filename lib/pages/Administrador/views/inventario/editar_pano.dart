...
import 'package:inventario/pages/Administrador/views/inventario/ver_panos.dart';
import 'package:inventario/services/database_services.dart';

class EditarPano extends StatefulWidget {
  final String panoId;
  final String marca;
  final String correo;
  ...
  @override
  State<EditarPano> createState() => _EditarPanoState();
}

class _EditarPanoState extends State<EditarPano> {
  final _formKey = GlobalKey<FormState>();
  final _formKeyPano= GlobalKey<FormState>();
  ...
  try {
    print("Actualizando paño con ID: ${widget.panoId}");
    print("Datos: $panoData");

    await _databaseServices.updatePano(widget.panoId, panoData);
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al actualizar paño: $e')),
      );
    }
  } finally {
    if (mounted) {
...
void _eliminarPano() {
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      content: const Text('¿Estás seguro de que deseas eliminar este paño? Esta acción no se puede deshacer.'),
      actions: [
        TextButton(
        ...
        ElevatedButton(
          onPressed: () async {
            try {
              await _databaseServices.deletePano(widget.panoId);
              if (mounted) {
                Navigator.pop(context);
                widget.onNavigate(ListaPanos(onNavigate: widget.onNavigate));
                ScaffoldMessenger.of(context).showSnackBar(
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Error al eliminar paño: $e')),
              );
            }
          },
...
Widget build(BuildContext context) {
        ...
        children: [
          ...
          Center(
            child: Wrap(
              ...
              children: [
                _buildCardEditar(Icons.edit, 'Editar Paño', _mostrarFormularioEdicion),
                _buildCardVer(Icons.visibility, 'Ver Paños', () => _verPanos(widget.panoId, widget.marca)),
                _buildCardAgregar(Icons.add, 'Agregar Paño', _agregarPano),
              ],
            ),
          ),
          ...
          Center(
            child: IconButton(
              ...
              onPressed: _eliminarPano,
              tooltip: 'Eliminar Paño',
            ),
          ),
        ],
      ),
 ...
 Widget _buildCardEditar(IconData icon, String text, VoidCallback onTap) {
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _isSaving ? null : _editarPano,
  ...
  void _verPanos(String panoId, String tipo) {
    widget.onNavigate(VerPanos(
    onNavigate: widget.onNavigate, 
    panoId: panoId,
    tipo: tipo, 
  ));
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        if (!_formKeyPano.currentState!.validate()) return;
