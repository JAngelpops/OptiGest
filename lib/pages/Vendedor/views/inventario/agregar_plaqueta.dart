...
class AgregarPlaqueta extends StatefulWidget {
  final Function(Widget) onNavigate;

  const AgregarPlaqueta({super.key, required this.onNavigate});

  @override
  State<AgregarPlaqueta> createState() => _AgregarPlaquetaState();
}

class _AgregarPlaquetaState extends State<AgregarPlaqueta> {
  ...
  void _guardarPlaqueta() async {
    ...
    try {
      await _databaseServices.addPlaqueta(plaquetaData);
      widget.onNavigate(Plaquetas(onNavigate: widget.onNavigate));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al guardar la plaqueta: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Agregar plaqueta', style: TextStyle(color: Colors.black, fontSize: 24, fontWeight: FontWeight.bold)),
        centerTitle: true,
