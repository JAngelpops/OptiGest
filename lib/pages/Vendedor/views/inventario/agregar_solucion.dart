...
class AgregarSolucion extends StatefulWidget {
  final Function(Widget) onNavigate;

  const AgregarSolucion({super.key, required this.onNavigate});

  @override
  State<AgregarSolucion> createState() => _AgregarSolucionState();
}

class _AgregarSolucionState extends State<AgregarSolucion> {
  ...
  void _guardarSolucion() async {
    ...
    try {
      await _databaseServices.addSolucionesLimpiadoras(solucionData);
      widget.onNavigate(SolucionesLimpiadoras(onNavigate: widget.onNavigate));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al guardar la solución: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Agregar solución', style: TextStyle(color: Colors.black, fontSize: 24, fontWeight: FontWeight.bold)),
        centerTitle: true,
