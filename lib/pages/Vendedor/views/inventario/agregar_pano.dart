...
class AgregarPano extends StatefulWidget {
  final Function(Widget) onNavigate;
  const AgregarPano({super.key, required this.onNavigate});

  @override
  State<AgregarPano> createState() => _AgregarPanoState();
}

class _AgregarPanoState extends State<AgregarPano> {
  ...
  void _guardarPano() async {
    ...
    try {
      await _databaseServices.addPano(panoData);
      widget.onNavigate(Panos(onNavigate: widget.onNavigate));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al guardar el paño: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Agregar paño', style: TextStyle(color: Colors.black, fontSize: 24, fontWeight: FontWeight.bold)),
        centerTitle: true,
