...
class AgregarTornillo extends StatefulWidget {
  final Function(Widget) onNavigate;

  const AgregarTornillo({super.key, required this.onNavigate});

  @override
  State<AgregarTornillo> createState() => _AgregarTornilloState();
}

class _AgregarTornilloState extends State<AgregarTornillo> {
  ...
  void _guardarTornillo() async {
    ...
    try {
      await _databaseServices.addTornillo(tornilloData);
      widget.onNavigate(Tornillos(onNavigate: widget.onNavigate));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al guardar el tornillo: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      resizeToAvoidBottomInset: true, // 🔹 Evita error de overflow con el teclado
      appBar: AppBar(
        title: const Text('Agregar tornillo', style: TextStyle(color: Colors.black, fontSize: 24, fontWeight: FontWeight.bold)),
        centerTitle: true,
            ...
            child: SingleChildScrollView(
              child: ConstrainedBox(
                constraints: BoxConstraints(
                  maxHeight: MediaQuery.of(context).size.height * 0.9, // 🔹 Evita desbordamiento
                ),
                child: Column(
