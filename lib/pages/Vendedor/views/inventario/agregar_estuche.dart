...
class AgregarEstuche extends StatefulWidget {
  final Function(Widget) onNavigate;
  const AgregarEstuche({super.key, required this.onNavigate});

  @override
  State<AgregarEstuche> createState() => _AgregarEstucheState();
}

class _AgregarEstucheState extends State<AgregarEstuche> {
  ...
  void _guardarEstuche() async {
    ...
    try {
      await _databaseServices.addEstuche(estucheData);
      widget.onNavigate(Estuches(onNavigate: widget.onNavigate));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al guardar el estuche: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Agregar estuche', style: TextStyle(color: Colors.black, fontSize: 24, fontWeight: FontWeight.bold)),
        centerTitle: true,
